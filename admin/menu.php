<ul class="sidebar-menu">
  <li class="active">
    <a href="dashboard.php">
        <i class="fa fa-dashboard"></i><span>Dashboard</span>
    </a>
  </li>
  <li class="treeview">
    <a href="">
        <i class="fa fa-users"></i>
        <span>Data Admin</span>
        <i class="fa fa-angle-left pull-right"></i>
    </a>
    <ul class="treeview-menu">
        <li><a href="admin.php"><i class="fa fa-angle-double-right"></i> Data Admin</a></li>
        <li><a href="admin-input.php"><i class="fa fa-angle-double-right"></i> Tambah Admin</a></li>
    </ul>
  </li>
  <li class="treeview">
    <a href="">
        <i class="fa fa-user"></i>
        <span>Data Jenis Buku</span>
        <i class="fa fa-angle-left pull-right"></i>
    </a>
    <ul class="treeview-menu">
        <li><a href="jenis.php"><i class="fa fa-angle-double-right"></i> Data Jenis Buku</a></li>
        <li><a href="jenis-input.php"><i class="fa fa-angle-double-right"></i> Tambah Jenis Buku</a></li>
    </ul>
  </li>
  <li class="treeview">
    <a href="">
      <i class="fa fa-user"></i>
      <span>Data Penerbit</span>
      <i class="fa fa-angle-left pull-right"></i>
    </a>
    <ul class="treeview-menu">
        <li><a href="penerbit.php"><i class="fa fa-angle-double-right"></i> Data Penerbit Buku</a></li>
        <li><a href="penerbit-input.php"><i class="fa fa-angle-double-right"></i> Tambah Penerbit Buku</a></li>
    </ul>
  </li>

                                <li class="treeview">
                                  <a href="">
                                      <i class="fa fa-user"></i>
                                      <span>Data Rak</span>
                                      <i class="fa fa-angle-left pull-right"></i>
                                  </a>
                                      <ul class="treeview-menu">
                                          <li><a href="rak.php"><i class="fa fa-angle-double-right"></i> Data Rak</a></li>
                                          <li><a href="rak-input.php"><i class="fa fa-angle-double-right"></i> Tambah Rak</a></li>
                                      </ul>
                                  </li>

                                <li class="treeview">
                                <a href="">
                                    <i class="fa fa-book"></i>
                                    <span>Data Buku</span>
                                    <i class="fa fa-angle-left pull-right"></i>
                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="buku.php"><i class="fa fa-angle-double-right"></i> Data Buku</a></li>
                                        <li><a href="buku-input.php"><i class="fa fa-angle-double-right"></i> Tambah Buku</a></li>
                                    </ul>
                                </li>





								<li class="treeview">
                                <a href="">
                                    <i class="fa fa-user"></i>
                                    <span>Data Kategori Anggota</span>
                                    <i class="fa fa-angle-left pull-right"></i>
                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="kategori-anggota.php"><i class="fa fa-angle-double-right"></i> Data Kategori Anggota</a></li>
                                        <li><a href="kategori-anggota-input.php"><i class="fa fa-angle-double-right"></i> Tambah Kategori Anggota</a></li>
                                    </ul>
                                </li>

                                <li class="treeview">
                                <a href="">
                                    <i class="fa fa-user"></i>
                                    <span>Data Anggota</span>
                                    <i class="fa fa-angle-left pull-right"></i>
                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="anggota.php"><i class="fa fa-angle-double-right"></i> Data Anggota</a></li>
                                        <li><a href="anggota-input.php"><i class="fa fa-angle-double-right"></i> Tambah Anggota</a></li>
                                    </ul>
                                </li>

								<li class="treeview">
                                <a href="">
                                    <i class="fa fa-user"></i>
                                    <span>Pengunjung</span>
                                    <i class="fa fa-angle-left pull-right"></i>
                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="pengunjung.php"><i class="fa fa-angle-double-right"></i> Data Pengunjung</a></li>
									</ul>
                                </li>

								<li class="treeview">
                                <a href="">
                                    <i class="fa fa-user"></i>
                                    <span>Transaksi</span>
                                    <i class="fa fa-angle-left pull-right"></i>
                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="transaksi.php"><i class="fa fa-angle-double-right"></i> Data Transaksi</a></li>
                                        <li><a href="transaksi-input.php"><i class="fa fa-angle-double-right"></i> Tambah Transaksi</a></li>
                                    </ul>
                                </li>

								<li class="treeview">
                                <a href="">

                                    <span>Laporan</span>

                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="buku-laporan2.php" target ="_blank">Laporan Buku</a></li>
										<li><a href="pengunjung-laporan.php">Laporan Pengunjung</a></li>
										<li><a href="input-informasi.php"> Laporan Pengunjung</a></li>
                                    </ul>
                                </li>

                            </ul>
